...
public class RichHtmlHandler {
	...
	public void handledHtml(boolean isWebApplication)
		...
		for (Element item : imageElement) {
			...
			/*if (imagHeightStr.endsWith("pt")) {
				imageHeight = Integer.parseInt(imagHeightStr.substring(0, imagHeightStr.length() - 2));
			} else {
				imageHeight = Integer.parseInt(imagHeightStr);
			}

			if (imagWidthStr.endsWith("pt")) {
				imageWidth = Integer.parseInt(imagWidthStr.substring(0, imagWidthStr.length() - 2));
			} else {
				imageWidth = Integer.parseInt(imagWidthStr);
			}*/
			// 得到文件的word mht的body块
